...
# COUNT OF VALUES ADDED (USED WHILE DELETING)
global val_added
val_added = 2
...
def fetch_val():
    ...
    print("Fetch Completed: " + str(datetime.now()-stmp))
...
def start():
    ...
    schedule.every(15).seconds.do(update_loop).tag('real-time-update')
...
def auto_delete():
    ...
    print(val_added)
    for sym in stock_sym:
        c.execute(("DELETE FROM {} ORDER BY DATE LIMIT {}").format(sym, val_added))
